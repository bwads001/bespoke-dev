...
class Task(BaseModel):
    ...
    task_id: str = Field(..., description="Unique task identifier (e.g., FE-01, BE-01)")
    task_type: str = Field(..., enum=["frontend", "backend", "database", "devops", "testing"])
...
async def analyze_task(task: str, workflow_conversation: List[Dict]) -> Tuple[List[Dict], Backlog]:
    ...
    analyst_conversation = workflow_conversation.copy()

    # Add the system prompt to the conversation
    analyst_conversation.append({'role': 'system','content': ANALYST_SYSTEM_PROMPT})

    # Add the user prompt to both conversations
    analyst_conversation.append({'role':'user', 'content':f"Break down this coding task into logical implementation steps: {task}"})
    ...
    try:
        ...
        # Add the assistant response to both conversations
        analyst_conversation.append({'role':'assistant', 'content':analyst_response})
        workflow_conversation.append({'role':'assistant', 'content':analyst_response})
        console.print("\n[green]Analysis complete![/green]")

        # Generate backlog
        console.print("\n[yellow]Generating task backlog...[/yellow]")
        backlog_response = await client.generate(
            model="qwen2.5-coder:14b",
            system=BACKLOG_SYSTEM_PROMPT,
            format=Backlog.model_json_schema(),
            options={'temperature': 0.2, 'top_k': 60, 'top_p': 0.3},
            prompt=f"Generate tasks for: {analyst_response}"
        )
        print(f"{GREY}{backlog_response.response}{RESET}", end="", flush=True)


        validated_backlog = Backlog.model_validate_json(backlog_response.response)
